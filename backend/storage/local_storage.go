...
func NewCacheStore(appName, filename string) *cacheStorage {
	return &cacheStorage{
		ConfPath: path.Join(userdir.GetDataHome(), appName, filename),
	}
}

// NewLocalStore returns a localStore instance.
func NewLocalStore(appName string, filename string) *localStorage {
	return &localStorage{
		ConfPath: path.Join(userdir.GetConfigHome(), appName, filename),
	}
}
